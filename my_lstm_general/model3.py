...
def fit_lstm2(train, batch_size, nb_epoch, neurons, timesteps):
    ...
    model.add(LSTM(neurons, activation='tanh', inner_activation='hard_sigmoid', input_shape=(X.shape[1], X.shape[2]), recurrent_dropout=0.2, return_sequences=True)) # <------------------------ instead of '100' was 'neurons'
...
def experiment(series, timesteps, shift):
    ...
    supervised_values = supervised.values[timesteps:, :].reshape(len_dataset - timesteps, timesteps - shift + 1,
                                                                 num_col)  #   ...-timesteps-shift
